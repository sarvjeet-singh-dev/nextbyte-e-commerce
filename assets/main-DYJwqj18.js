...
`,s=()=>`
  ...
  <div id="products-card-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mt-12">
    ...
    `,V=()=>{let t=e();if(F.isLoading){t.featuredProducts.innerHTML=B();return}if(F.products.length===0){t.featuredProducts.innerHTML=`<p>No Products Found</p>`;return}let n=e();F.filterdProducts.length===0?n.productsCardGrid.innerHTML=R():n.productsCardGrid.innerHTML=L()},H=()=>{let t=e();F.selectedCategory===`Creator Gear`||F.selectedCategory===`Portable Tech`?t.productsFilter.value=`Accessories`:t.productsFilter.value=F.selectedCategory,t.productsFilter.dispatchEvent(new Event(`change`))},U=()=>{let t=e();if(F.isLoading){t.featuredProducts.innerHTML=B();return}if(F.products.length===0){t.featuredProducts.innerHTML=`<p>No Products Found</p>`;return}t.featuredProducts.innerHTML=z();let n=e();n.productsCardGrid.innerHTML=R()},ie=()=>{e().categories.addEventListener(`click`,e=>{let t=e.target.closest(`.categories-cards`);t&&(F.selectedCategory=t.dataset.name,H(),document.getElementById(`featured-products`)?.scrollIntoView({behavior:`smooth`}))})},W=()=>{e(),document.addEventListener(`click`,e=>{let t=e.target.closest(`.footer-links`);if(t){e.preventDefault();let n=()=>document.getElementById(`home-page`)!==null,r=t.dataset.category;n()?(F.selectedCategory=r,H(),document.getElementById(`featured-products`)?.scrollIntoView({behavior:`smooth`})):(sessionStorage.setItem(`selectedCategory`,r),window.location.href=`/nextbyte-e-commerce/#featured-products`)}let n=e.target.closest(`.social-links`);if(n){let e=n.dataset.name;A(`${e} Clicked!`,` Implementation coming soon.`,3e3)}})},G={itemDetails:void 0,itemMainImage:void 0},K=()=>G.itemDetails.images.map((e,t)=>`
